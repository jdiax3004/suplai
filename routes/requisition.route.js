...
router.get("/requisitions/stats", async (req, res) => {
  let filters = req.query ? req.query : {};
  let requisicion = await Requisition.find(filters);
  return res.status(200).json(requisicion);
});
...
router.post("/requisition", ensureAuthenticated, (req, res) => {
  ...
  if (errors.length !== 0) {
    ...
    newReq
      .save()
      .then((req) => {
        res.status(201).json("New Requisition Added: " + title);
...
router.put("/requisition/:id", ensureAuthenticated, async (req, res) => {
  try {
    ...
    if (req.user.type == "BOSS") {
      ...
      const financerID = await User.findOne({ tier: tierUser }, function (
        err,
        docs
      ) {});
...
async function sendNewRequisition(newReq) {
  ...
  const requisitionurl = "http://localhost:4200/requisicion/" + newReq._id;
  ...
  let info = transporter.sendMail({
    ...
    subject:
      owner.name + ", Ha creado una Requisicion nueva. [ID:" + newReq._id + "]", // Subject line
    /* text:
     *         "", */
    // plain text body
...
//[GET] Read Requisitions
router.get("/requisitions/table", ensureAuthenticated, async (req, res) => {
	console.log(req.user.type);
	try {
	  var requisitions;
	  if (req.user.type.toUpperCase() == "ADMIN") {
