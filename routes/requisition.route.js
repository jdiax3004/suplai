...
router.get("/requisitions/:id", ensureAuthenticated, async (req, res) => {
  try {
    var requisition = await Requisition.findById(req.params.id).populate({
      path: "owner",
      populate: {
        path: "boss"
      }
...
router.post("/requisition", ensureAuthenticated, (req, res) => {
  ...
  if (errors.length !== 0) {
    ...
    newReq
      .save()
      .then(req => {
        res.status(201).send("New Requisition Added: " + title);
  ...
  try {
    ...
    if (req.user.type == "BOSS") {
      ...
      const requisition = await Requisition.findByIdAndUpdate(
        req.params.id,
        { checker: financerID._id, status: req.body.status },
        {
          new: true
        }
      );
